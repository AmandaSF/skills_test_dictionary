...
def count_unique(input_string):
    ...
    for words in input_string.split():
        dict_unique[words] = dict_unique.get(words, 0)

    return dict_unique
    #use .get because .get gives a default value of None rather than a value error
...
def find_common_items(list1, list2):
    ...
    for item1 in list1:
        for item2 in list2:
            if item1 == item2:
                venn_intersection.append(item1)


    return venn_intersection
...
def get_sum_zero_pairs(input_list):
    ...
    return [[-x, x] for x in number_set if x >=0 and -x in number_set]
...
def remove_duplicates(words):
    ...
    """
    # this would be so much easier if I was allowed to use sets!
...
def encode(phrase):
    ...
    """
    code_dict = {'e': 'p', 'a': 'd', 't': 'o', 'i': 'u'}
    phrase_code = ""
    for character in phrase:
        if character in code_dict:
            phrase_code = phrase_code + code_dict[character]
        else:
            phrase_code = phrase_code + character
...
#also sorted method can be reversed really easily by using reverse = true
...
def get_pirate_talk(phrase):
    ...
    return ' '.join([encode_to_pirate.get(word, word) for word in phrase.split()])
